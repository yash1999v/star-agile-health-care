    ...
    stages {
        stage('SCM Checkout') {
            steps {
                ...
                git 'https://github.com/yash1999v/star-agile-health-care.git'
       ...
       stage("Docker build"){
            steps {
				sh 'docker version'
				sh "docker build -t yash1999v/healthapp:${BUILD_NUMBER} ."
				sh 'docker image list'
				sh "docker tag yash1999v/healthapp:${BUILD_NUMBER} yash1999v/healthapp:latest"
            }
        }
		...
		stage('Push2DockerHub') {
			...
			steps {
				sh "docker push yash1999v/healthapp:latest"
			}
		}
